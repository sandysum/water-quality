...
mod_as_ej_income <- felm(pws_as_median ~ log_hh_income | 0 | 0 | 0, data = as_pws_10yr)

summary(mod_as_ej_income)

mod_as_ej_percent_his <- felm(pws_as_median ~ perc_latino | 0 | 0 | 0, data = as_pws_10yr)
...
n_pws_10yr %>% ungroup() %>% ggplot(aes(median_hh_income, pws_n_median)) +
  geom_point(alpha = .5) +
  geom_smooth() +
  theme_minimal()
...
mod_n_ej_percent_his <- felm(pws_n_median ~ perc_latino | 0 | 0, data = n_pws_10yr)
