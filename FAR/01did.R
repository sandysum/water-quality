...
df2 <- readRDS('../Data/1int/usa_inf_deaths.rds') %>% 
  left_join(w.year, by = c('GEOID', 'year'))
...
pws <- read_csv("../Data/SDWA-DL/SDWA_PUB_WATER_SYSTEMS.csv") %>% 
  ...
  # filter(PWS_ACTIVITY_CODE != "I", STATE_CODE == 'CA') %>% 
...
as_vio_new %>%
  ...
  summarise(sum.as = sum(as_violations, na.rm = TRUE),
            sum.pop = sum(POPULATION_SERVED_COUNT, na.rm = TRUE)) %>% 
  arrange(desc(sum.as)) %>% 
  filter(year == 2012)

as_vio_new %>%
  filter(!is.na(STATE_CODE)) %>% 
  ggplot(aes(year, POPULATION_SERVED_COUNT, group = STATE_CODE)) +
  stat_summary(fun.y = sum,
               geom = "line", color = 'grey70', alpha = .8) +
  # stat_summary(fun.y = sum,
  #              geom = "point") +
  scale_color_brewer(palette = 'Dark2') +
  labs(x='Year', title='Population served with As > 10 ppb', y = ' ') +
...
df2 %>%
  left_join(as.counties %>% mutate(
    as_bins = cut_number(usgs_as, 3, c('low', 'median', 'high'))
  ), by  = 'GEOID') %>% 
  filter(!is.na(as_bins)) %>% 
  ggplot(aes(year, death.rates)) +
  stat_summary(fun = median, aes(color = as_bins),
               geom = "line",
               alpha = .3, size = 1) +
  stat_summary(fun = median,
               ...
               size = 1) +
  scale_color_brewer(palette = 'Dark2') +
  labs(x='Year', y='Infant mortality rate', color = 'Mean county \narsenic bin') +
  geom_vline(xintercept = c(2001, 2006), color = 'red') +
  theme_minimal_hgrid()
...
df3 <- df2 %>% 
  # filter(year>200) %>%
  left_join(as.counties %>% mutate(
  as_bins = if_else(usgs_as >= 10, 'high', 'low')
) %>% select(matches('as'), GEOID)) %>% 
  mutate(treat = if_else(as_bins == 'high', 2010, 0),
         postXh = if_else(year >= 2010 & as_bins == 'high', 1, 0),
         stateXyear = paste0(state,year))

femod1 <- feols(death.rates ~ postXh + PM25_conc + tmax + O3_conc | state + county_fips + year, df3)
summary(femod1)

femod2 <- feols(death.rates ~ postXh + PM25_conc + tmax + O3_conc | state + county_fips[year], df3)
summary(femod2)

femod3 <- feols(death.rates ~ postXh + PM25_conc + tmax + O3_conc | state[year] + county_fips, df3)
summary(femod3)
...
df3 <- df2 %>% 
  ...
  mutate(treat = if_else(as_bins == 'high', 2006, 0),
...
example_attgt <- att_gt(yname = "death.rates",
                        ...
                        idname = "county_fips",
                        gname = "treat",
                        xformla = ~PM25_conc + county_fips + state + tmax,
                        data = df3
)
